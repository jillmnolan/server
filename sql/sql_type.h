...
public:
  ...
  uint32 max_display_length(const Item *item) const;
/* 
    The next method returns 308 for long stringified doubles in scientific 
    notation, e.g. FORMAT('1e300', 2).
*/ 
  uint32 Item_decimal_notation_int_digits(const Item *item) const { return 308; }
  bool Item_const_eq(const Item_const *a, const Item_const *b,
                     bool binary_cmp) const;
  bool Item_eq_value(THD *thd, const Type_cmp_attributes *attr,
                     Item *a, Item *b) const;
